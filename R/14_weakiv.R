...
cat("Partialling reproduces the 2SLS estimates, and the fast Anderson and Rubin curve\n",
...
txt <- c(sprintf("%-16s %-14s %3s %9s %9s %-20s %-22s %-22s",
                 ...
                 "2SLS (SE)", "Conventional 95% CI", "Anderson and Rubin 95%"))
...
txt <- c(txt, "",
         sprintf("Thresholds: Stock and Yogo 10%% maximal size = %s (by instrument count);",
                 commas(vapply(sort(unique(tab$k)), function(x) sy10(1L, x), numeric(1)), 2)),
         sprintf("Montiel Olea and Pflueger, worst case for 10%% Nagar bias = %.2f.", MOP_CV_10),
         "", "95% confidence sets:",
...
txt <- c(txt, "", "Anderson and Hsiao columns (two endogenous regressors, no CLR counterpart):",
         sprintf("%-16s %-16s %11s %11s %13s %9s", "Spec", "Panel", "F: democracy",
                 "F: income", "Cragg and Donald", "SY 10%"))
...
sprintf(paste(
  ...
  "Anderson and Rubin confidence sets and the Moreira conditional likelihood ratio",
...
paste(
  ...
  "instrumental variables columns. They are the Anderson and Hsiao and Arellano and Bond",
  "columns, which instrument two variables at once. The confidence sets here have no",
  "direct counterpart there, so I report first stage diagnostics for those columns."),
...
paste(
  ...
  "trustworthy, since they rest on the assumption that the instrument has real pull.",
  "The tests below stay honest whether the instrument is strong or weak. Comparing",
  "them against the ordinary interval shows how much the ordinary interval relied on",
  "that assumption."),

"## How strong is the first stage",
sprintf(paste(
  "The first stage F statistics run from %.1f to %.1f. I cluster them by country,",
...
paste(
  "That last comparison is not strictly legitimate, and last round I said so and left",
  "it there. Stock and Yogo derive their cutoffs for well behaved, unclustered errors,",
  "while I cluster these F statistics by country. Under clustering, the properly",
  "justified statistic is the effective F of Montiel Olea and Pflueger (2013). This",
  "round I implement it, and it appears in the table beside the ordinary one."),
sprintf(paste(
  "I have two observations about what the effective F changes. First, for the %s",
  "columns with a single instrument, the effective F is algebraically identical to the",
  "cluster robust first stage F. The script checks this identity at run time. So",
  "nothing moves there."), spell(n_single)),
sprintf(paste(
  ...
  "unevenly the instruments contribute, which is something the ordinary F ignores."),
  spell(n_multi),
...
paste(
  ...
  "several of them are not comfortably strong. The two confidence sets below handle",
  "exactly that situation."),

"## Anderson and Rubin",
paste(
  "The Anderson and Rubin test asks a simple question. The test takes a candidate value",
...
sprintf(paste(
  ...
  "Anderson and Rubin statistic tends to the first stage F as the candidate value runs off",
...
sprintf(paste(
  ...
  "of [%.2f, %.2f]. The Anderson and Rubin set is at least as wide as the ordinary interval",
  "in every column, so the ordinary intervals are optimistic throughout."),
...
sprintf(paste(
  "I should not push that comparison further than it goes. The columns do not form a",
  "tidy order by first stage strength. The widening ratio runs from %.2f to %.2f, and",
  "it does not decrease neatly as the first stage gets stronger."),
  min(widen$ratio), max(widen$ratio)),
sprintf(paste(
  "%s has the weaker first stage of the two, an effective F of %.1f against %.1f. It",
  "still widens less than %s does, with widening ratios of %.2f against %.2f. These",
  "specifications differ in their controls and samples as well as in instrument",
  "strength, so the two ends of the range are suggestive and no more than that."),
...
paste(
  ...
  "Instead of comparing the statistic against a fixed cutoff, it adjusts the cutoff",
  "with a quantity that carries the information about instrument strength. That",
  "adjustment buys back power that Anderson and Rubin gives away. I coded it directly,",
  "so one grid inversion serves all three tests here. R/15_ivcrosscheck.R checks it",
  "against the ivmodel package, which implements the same test."),
sprintf(paste(
  ...
  "and the script uses that form directly."),
  spell(n_single), spell(nrow(tab)), spell(n_multi)),
paste(
  "Moreira's test assumes that the errors are well behaved and independent, and the",
  "packages implement it that way. The paper clusters, and the Anderson and Rubin sets",
  "above cluster. So last round I reported the two side by side, with a caveat. My",
  "caveat was that the gap between them was mostly a matter of clustering. That was a",
  "conjecture."),
paste(
...
sprintf(paste(
  ...
  "columns the cluster robust statistic equals the clustered Anderson and Rubin statistic.",
  ...
  "distributions. The likelihood ratio test uses chi squared and the Anderson and Rubin test",
  "uses F. So for those columns the entire gap between the homoskedastic set and the",
  "Anderson and Rubin set was the clustering, as I guessed."),
  spell(n_single), cal_gap),
sprintf(paste(
  "For the %s columns with two instruments the answer is more interesting, and it",
  "surprised me. The cluster robust version widens the set by a factor of %.2f",
  "and %.2f. The result still sits strictly inside the Anderson and Rubin set:",
...
paste(
  ...
  "absent once the fixed differences between countries come out. What the result does",
  "mean is that the instrumental variables columns are uninformative about the sign of",
  "the effect, and that they provide no evidence for a negative effect."),
...
paste(
  ...
  "counterpart at all."),
paste(
  "The Arellano and Bond columns add a third reason. Their instrument counts run into the",
  "dozens against about a hundred countries, and this sample cannot support a",
  "cluster robust weight matrix of that size. That point deserves emphasis, since it",
  "is the subject of docs/instruments.md, where the same instrument counts inflate the",
  "apparent precision of the estimates."),
sprintf(paste(
  "The first stage diagnostic does transfer, so I report it for the Anderson and Hsiao",
  "columns, %s. With two endogenous regressors the right statistic is Cragg and",
  "Donald's minimum eigenvalue rather than a per equation F. The Stock and Yogo cutoff",
  "changes with it, to %.2f. The Cragg and Donald statistics are %s and %s, so both clear",
...
paste(
  "The Cragg and Donald statistic is the homoskedastic one, so it carries the same caveat",
  "as the Stock and Yogo comparison above. There is no clustered version of the cutoff to",
  "hold it against. Taken together these are not the weakest instruments in the paper.",
  "Those columns still produce very wide intervals, and the explanation appears to be",
  "the small number of moment conditions that they use."),
...
paste(
  ...
  "published two stage least squares estimates. The fast Anderson and Rubin curve must",
...
paste(
  ...
  "must equal the clustered Anderson and Rubin test. The effective F must equal the",
  "ordinary F in the same case. Every Anderson and Rubin set must contain its own point",
