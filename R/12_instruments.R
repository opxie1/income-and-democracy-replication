...
sprintf(paste(
  ...
  "from last round, so that the lag window and the collapsing choice now move together",
  "in one picture. These estimators use the past of a country as a stand in for its",
  "present, so the instruments are old values of democracy and income. Without",
  "collapsing, the estimator gets one instrument for each pair of a lag and a time",
  "period. With collapsing, it gets one instrument per lag distance, so the count is",
  "much smaller. I run both, for lag windows of %d through %d, for both GMM estimators",
  "and both democracy measures."), min(LAG_WINDOW), max(LAG_WINDOW)),
paste(
  "A third dial turns out to matter more than either of these, so it is on the figure",
  "as well. The sweep from last round used the two step weighting matrix, and I treated",
  "that step as settled. This round the weighting step is a dial on the figure, and it",
  "is the choice that decides the answer. Results are in output/instruments.txt,",
...
sprintf(paste(
  ...
  "Difference and system GMM use different samples from each other. For %s",
  "they use %d and %d countries, since system GMM can also use the level equations.",
...
sprintf(paste(
  "The instrument count increases quickly with the window, although not without",
...
paste(
  ...
  "fails for the other. Since the sweep from last round used two step weighting, the",
  "puzzle I reported then appears to be a fact about the weighting step."),
sprintf(paste(
  ...
  "uncollapsed lines drift toward the fixed effects values of %.3f and %.3f, while the",
  "collapsed lines stay where they started."),
...
sprintf(paste(
  ...
  "by %.3f against %.3f uncollapsed, so collapsing does nothing to slow the drift.",
...
paste(
  "The reason appears to be the two step weighting matrix itself. The estimator builds",
  "that matrix from the residuals, and the matrix has one row and one column per",
  "instrument. With a wide window and a few dozen countries, the data are too thin to",
  "estimate the matrix well, and the matrix then absorbs noise of its own. Collapsing",
  "makes the matrix smaller, although evidently not small enough here."),
paste(
  "So the honest summary is that collapsing does its advertised job. The two step",
  "weighting step is a separate form of overfitting, and collapsing leaves it in place.",
  "A reader who looks only at the two step columns would attribute a problem to",
  "collapsing that originates elsewhere."),
sprintf(paste(
  ...
  "everywhere, and the largest p value across the whole two step sweep is %s. Those",
  "level conditions therefore look invalid on this data, whatever the instrument count."),
  po, num(sys_worst, 3)),
...
sprintf(paste(
  ...
  "That sharpness is a product of the instrument count alone."),
...
paste(
  ...
  "two stage least squares columns. A conventional standard error assumes that the",
...
paste(
  ...
  "plm package reports an overidentification test after either estimator, and by",
  "default it builds the statistic from the robust weight matrix in both cases. The",
  "number it prints after a one step fit is therefore something other than the",
  "criterion of that estimator. I report the criterion of each estimator instead."),
sprintf(paste(
  "The %s statistic sits in the one step rows, and it assumes homoskedastic errors.",
  "The %s statistic sits in the two step rows. It is robust to heteroskedasticity,",
  "although it loses power as the instrument count increases. They are different tests,",
  "so a reader should compare p values down a column and not across the weighting rows."),
...
sprintf(paste(
  ...
  "uncollapsed sets, at every window, for both measures. This is what a",
  "homoskedastic test does on data with heteroskedastic errors. The errors",
...
sprintf(paste(
  ...
  "clean. The test loses power as the set grows, while the instruments themselves stay",
  "as they were. A high %s p value on a large instrument set is therefore not",
  "reassurance."),
  OVERID_TEST[["twostep"]],
...
sprintf(paste(
  ...
  "%.2f, so nothing here suggests that the second lag instruments are invalid."),
...
sprintf(paste(
  ...
  "samples still differ between the two sweeps, although I no longer need that",
  "difference to explain anything."),
