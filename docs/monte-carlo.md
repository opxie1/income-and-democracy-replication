...
The idea is to build a dynamic panel that resembles the real one. It keeps
...
are known, so the error of each estimator can be measured. The results give a
...
effect and a shock. The two shocks are drawn together, so that a shock to one
variable can move the other. The country effects are drawn together as well,
with a correlation of 0.59 for Freedom House. This correlation is what makes
rich countries democratic for fixed reasons, and it is also what pooled OLS
mistakes for an effect of income.

The simulated panel keeps the shape of the real one. It reproduces the
observation pattern cell by cell, for each variable separately, so that the
...
Freedom House and 0.958 for Polity, both taken from difference GMM. This
number turns out to matter, and I return to it in the results.

The true effect of income on democracy is zero in every design. This choice
...
equal the real one, 0.600 and 0.702. The second calibration asks only which
value of true persistence produces the fixed effects number that the data
show.
...
One feature of the data does not match, and it is worth stating. Pooled OLS
is more biased in the simulation than it is in the data. A linear model with
a country effect and a normal shock cannot match three things at once, namely
the spread of countries, the fixed effects estimate and the pooled OLS
estimate. The simulation is therefore a slightly harsh world for the
estimators that work in levels. Democracy in the real data also sits between
...
deviation across draws is 0.117. The uncollapsed estimator has a standard
deviation of only 0.071, and an average error of -0.098. Its intervals cover
the truth in 0.73 of draws. This is the cost of too many instruments,
measured directly.
...
1 per cent for Polity. A number of this size can arise on data of this shape
...
is that the bias is real and specific to the measure, and that I cannot yet
name its source.
...
estimator closest to that target is Blundell and Bond, system GMM (two step)
at 0.96. Pooled OLS is the worst at 0.00.
...
thing to say. On persistence, pooled OLS and fixed effects both perform
poorly. They miss by 0.289 and -0.215 for Freedom House, and neither interval
ever covers the truth. Collapsed difference GMM is the only estimator that is
close to unbiased while remaining honest about its own uncertainty.

The income coefficient gives a different picture. Pooled OLS still fails,
with coverage of 0.00. Fixed effects performs well here, with an error of
-0.036 and intervals that cover the truth 0.90 of the time. The bias of the
fixed effects estimator falls mostly on the lagged dependent variable. So a
reader who cares only about the income coefficient loses less by using fixed
effects than the persistence results suggest.
...
root mean squared error still, at 0.041, which is the trap in using that
measure alone. Root mean squared error rewards a tight estimate even when it
is centered in the wrong place, so coverage has to be read alongside it.
...
all. Neither of them is consistent here at any sample size, so a larger panel
...
as its instrument, so that estimator has the least to work with. I suspect
that this reflects slow convergence, although the simulation here cannot
settle the question.
