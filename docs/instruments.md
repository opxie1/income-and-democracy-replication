...
sweep from last round, so that the lag window and the collapsing choice now
move together in one picture. These estimators use the past of a country as a
...
distance, so the count is much smaller. I run both, for lag windows of 2
through 8, for both GMM estimators and both democracy measures.

A third dial turns out to matter more than either of these, so it is on the
figure as well. The sweep from last round used the two step weighting matrix,
and I treated that step as settled. This round the weighting step is a dial
on the figure, and it is the choice that decides the answer. Results are in
output/instruments.txt, output/instruments.csv and
output/instruments_drift.csv, and the figure is output/instruments.png.
...
instrument set. Difference and system GMM use different samples from each
other. For Freedom House they use 124 and 133 countries, since system GMM can
also use the level equations. Their levels are therefore not strictly
...
it happens for one measure and fails for the other. Since the sweep from last
round used two step weighting, the puzzle I reported then appears to be a
fact about the weighting step.
...
0.010 and -0.006, while the collapsed lines stay where they started.

Two step weighting breaks that pattern for Freedom House. There the collapsed
estimate moves by 0.179 against 0.169 uncollapsed, so collapsing does nothing
to slow the drift. The collapsed line runs from -0.234 up to -0.055. The
...
The reason appears to be the two step weighting matrix itself. The estimator
builds that matrix from the residuals, and the matrix has one row and one
column per instrument. With a wide window and a few dozen countries, the data
are too thin to estimate the matrix well, and the matrix then absorbs noise
of its own. Collapsing makes the matrix smaller, although evidently not small
enough here.

So the honest summary is that collapsing does its advertised job. The two
step weighting step is a separate form of overfitting, and collapsing leaves
it in place. A reader who looks only at the two step columns would attribute
a problem to collapsing that originates elsewhere.
...
rejects it for Polity everywhere, and the largest p value across the whole
two step sweep is 0.047. Those level conditions therefore look invalid on
this data, whatever the instrument count.
...
window widens. That sharpness is a product of the instrument count alone.
...
assumes that the instruments have real pull, and it decreases whether or not
they do.
...
estimator, and by default it builds the statistic from the robust weight
matrix in both cases. The number it prints after a one step fit is therefore
something other than the criterion of that estimator. I report the criterion
of each estimator instead.
...
heteroskedasticity, although it loses power as the instrument count
increases. They are different tests, so a reader should compare p values down
a column and not across the weighting rows.
...
This is what a homoskedastic test does on data with heteroskedastic errors.
The errors here are heteroskedastic enough that the paper clusters all of its
own inference by country.
...
power as the set grows, while the instruments themselves stay as they were. A
high Hansen p value on a large instrument set is therefore not reassurance.
...
across all 112 fits is 0.17, so nothing here suggests that the second lag
...
the two sweeps, although I no longer need that difference to explain
anything.
